...
void vb3_body_apply_force( uint64_t body, double fx, double fy, double fz, double px, double py, double pz,
						  int wake )
{
	b3Body_ApplyForce( b3LoadBodyId( body ), (b3Vec3){ (float)fx, (float)fy, (float)fz },
					   (b3Pos){ (float)px, (float)py, (float)pz }, wake != 0 );
}

void vb3_body_set_damping( uint64_t body, float linear, float angular )
{
	b3Body_SetLinearDamping( b3LoadBodyId( body ), linear );
	b3Body_SetAngularDamping( b3LoadBodyId( body ), angular );
}

void vb3_body_enable_sleep( uint64_t body, int enable )
{
	b3Body_EnableSleep( b3LoadBodyId( body ), enable != 0 );
}

void vb3_body_set_sleep_threshold( uint64_t body, float threshold )
{
	b3Body_SetSleepThreshold( b3LoadBodyId( body ), threshold );
}

void vb3_body_set_gravity_scale( uint64_t body, float scale )
{
